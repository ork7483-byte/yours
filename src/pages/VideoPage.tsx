...
export default function VideoPage() {
  ...
  const [videoProvider, setVideoProvider] = useState<VideoProvider>('aiml');
  ...
  async function generateVideo() {
    ...
    try {
      if (videoProvider === 'aiml') {
        const key = await getApiKey('aiml_api_key');
        if (!key) throw new Error('AIML API 키가 설정되지 않았습니다. 관리자 대시보드에서 키를 입력해주세요.');
        ...
        if (!generationId) throw new Error('generation_id를 받지 못했습니다.');

        // Poll every 15 seconds, up to 20 attempts (5 minutes)
        for (let i = 0; i < 20; i++) {
          await delay(15000);
          const poll = await fetch(`https://api.aimlapi.com/v2/video/generations/${generationId}`, {
            headers: { Authorization: `Bearer ${key}` },
          });
          if (!poll.ok) continue;
          const pData = await poll.json();
          if (pData.status === 'completed' || pData.status === 'success') {
            const videoUrl = pData.video_url || pData.output?.video_url;
          ...
          if (pData.status === 'failed' || pData.status === 'error') {
            throw new Error(pData.error || '영상 생성에 실패했습니다.');
          }
        }
        ...
        // PiAPI
        const key = await getApiKey('piapi_key');
        if (!key) throw new Error('PiAPI 키가 설정되지 않았습니다. 관리자 대시보드에서 키를 입력해주세요.');

        const res = await fetch('https://api.piapi.ai/api/kling/v1/video', {
          method: 'POST',
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(err.message || `PiAPI 오류 (${res.status})`);
        }
        const data = await res.json();
        ...
        for (let i = 0; i < 20; i++) {
          await delay(15000);
          const poll = await fetch(`https://api.piapi.ai/api/kling/v1/video/${taskId}`, {
            headers: { 'x-api-key': key },
          });
          if (!poll.ok) continue;
          const pData = await poll.json();
          const status = pData.status || pData.data?.status;
          if (status === 'completed' || status === 'success') {
            const videoUrl = pData.video_url || pData.data?.video_url || pData.output?.video_url;
            if (!videoUrl) throw new Error('영상 URL을 받지 못했습니다.');
            setVideoResult(videoUrl);
            setVideoLoading(false);
            return;
    ...
    <div className="min-h-screen bg-[#FAFAF9] font-sans antialiased">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
              ...
              {allImages.map(img => {
                ...
                return (
                  <div
                    ...
                    className={`relative aspect-square rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${isSelected ? 'border-neutral-900' : 'border-transparent hover:border-neutral-300'}`}
                  >
        ...
        <Section icon={<Video className="w-4 h-4 text-violet-500" />} title="Step 2 — AI 영상 생성">
          <TabBar
            options={[
              { value: 'aiml', label: 'AI/ML API (Kling)' },
              { value: 'piapi', label: 'PiAPI (Kling)' },
            ]}
            value={videoProvider}
            onChange={v => setVideoProvider(v as VideoProvider)}
          />
